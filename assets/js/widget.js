function updateUserWidget(userData) {
    ...
    document.querySelectorAll(".widget").forEach(widget => {
        ...
        if (userData.posts && userData.posts.data && userData.posts.data.length > 0) {
            let postsHtml = userData.posts.data.map(post => `<p>${post.message || "(Sin texto)"}</p>`).join("<hr>");
        ...
        if (userData.photos && userData.photos.data && userData.photos.data.length > 0) {
        ...
        if (userData.videos && userData.videos.data && userData.videos.data.length > 0) {
